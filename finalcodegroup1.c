...
//INT0 (RB0) starts the robot; TMR0 counts ms
void interrupt(){
    if(INTCON & 0B00000010){
        start_flag = 1;
        INTCON = INTCON & 0B11110101;   // clear INTF
    }

    if((INTCON & 0B00000100) && (start_flag == 1)){
        TMR0 = 248;
        ms_counter++;
        INTCON = INTCON & 0B11111011;   // clear T0IF
    }
}

//  Hardware PWM via CCP1 (RC2) and CCP2 (RC1)
...
// a -> CCPR1L (left motor duty), b -> CCPR2L (right motor duty)
...
// Both wheels driven inward at equal speed (true centre pivot left)
...
// Both wheels driven inward at equal speed (true centre pivot right)
...
//  Servo on RB5: 25 software-PWM pulses at 20 ms period
...
// ADC read of LDR on AN0/RA0 (right-justified, 10-bit)
...
// HC-SR04 raw readers: trigger 20 us pulse, time echo with TMR1
// Returns distance in cm, or 999 on timeout.

unsigned int read_sonar_front_raw(void){       // trig RD0, echo RC3
...
unsigned int read_sonar_mid_raw(void){         // trig RD1, echo RD6
    unsigned int ticks, timeout;
    unsigned char snap;
...
unsigned int read_sonar_left_raw(void){        // trig RD2, echo RD7
    unsigned int ticks, timeout;
    unsigned char snap;
...
// Median-of-3 filter: rejects readings < 2 cm (noise floor)
...
// Three samples with 4 ms inter-sample gap to prevent echo crosstalk
...
void main(void){
    ...
    // P-controller gains and limits for wall following
    unsigned char kp             = 4;
    ...
    //Start sequence: wait for INT0, then 3 s TMR0 delay
    ...
    INTCON = INTCON & 0B11101101;       // disable INT0

    OPTION_REG = 0B11000111;            // TMR0: Fosc/4, 1:256 prescaler
    TMR0       = 248;
    ms_counter = 0;
    INTCON     = INTCON & 0B11111011;
    INTCON     = INTCON | 0B00100000;   // enable TMR0 overflow interrupt

    while(ms_counter < 3000){           // 3-second countdown
    }

    INTCON = INTCON & 0B11011111;       // disable TMR0 interrupt
    INTCON = INTCON & 0B11111011;       // clear T0IF
    ...
    while(1){
        ...
        //  Zone 4: stop, raise flag, hold servo active
        if(zone == 4){
            Move(0, 0);
            for(i; i < 1500; i++){
                asm nop;
        ...
        //LDR-based zone transitions (zones 1 and 2 only)
        if(zone == 1 || zone == 2){
            light_level = read_LDR();

            if(zone == 1 && light_level > DARKNESS_THRESHOLD){
                zone = 2;
                PORTD = PORTD | 0B00001000;     // buzzer ON
                light_counter = 0;
                Move(0, 0);
            }
            else if(zone == 2){
                if(light_level < DARKNESS_THRESHOLD){
                    light_counter++;
                    if(light_counter >= 5){         // 5 consecutive light readings
                        zone = 3;
                        PORTD = PORTD & 0B11110111; // buzzer OFF
                        light_counter = 0;
                        Move(60, 60);
                        my_delay_ms(350);           // forward push to exit tunnel
                        Move(0, 0);
                        my_delay_ms(60);
                    }
                }
                else {
                    light_counter = 0;              // reset if light returns
                }
            }
        }

        //  Zone 1: open-track line following with T-intersection
        if(zone == 1){
            right_sensor = (PORTD & 0B00010000);
            left_sensor  = (PORTD & 0B00100000);

            if(right_sensor && left_sensor){
                // Dwell and recheck to confirm T-intersection
                Move(50, 50);
                my_delay_ms(15);
                right_sensor = (PORTD & 0B00010000);
                left_sensor  = (PORTD & 0B00100000);
                ...
                if(right_sensor && left_sensor){
                    ...
                    // Phase 1: wait until right sensor leaves the bar
                    pivot_timeout = 0;
                    ...
                    // Phase 2: wait until right sensor finds the new arm
                    pivot_timeout = 0;
                    while( !(PORTD & 0B00010000) ){
                        pivot_timeout++;
                    ...
                    Move(60, 60);
                    my_delay_ms(90);            // forward commit to centre on new arm
                }
            }
            else if(right_sensor && !left_sensor){
                Move(80, 0);                    // drift left: steer right
            }
            else if(left_sensor && !right_sensor){
                Move(0, 80);                    // drift right: steer left
            }
            else {
                Move(60, 60);                   // both off: advance to reacquire
            }
        }

        //  Zone 2: tunnel line following
        else if(zone == 2){
            right_sensor = (PORTD & 0B00010000);
            left_sensor  = (PORTD & 0B00100000);
        ...
        // Zone 3: maze navigation
        else if(zone == 3){

            // Finish detection: both IR sensors on black for 5 consecutive reads
            right_sensor = (PORTD & 0B00010000);
            left_sensor  = (PORTD & 0B00100000);
            ...
            // Fast single-sample read; full median only if obstacle is close
            dist_front = read_sonar_front_raw();
            if(dist_front != 999 && dist_front < 17){
                dist_front = read_sonar_front();
            }

            // Obstacle avoidance: continuous pivot until path clears
            if(dist_front != 999 && dist_front < 22){
                Move(0, 0);
                my_delay_ms(180);

                // Sample sides once to select pivot direction
                dist_mid  = read_sonar_mid();
                dist_left = read_sonar_left();

                // High initial speed overcomes static friction
                ...
                // Continue pivot until front sonar reports clear (> 30 cm)
                // Raw read used here for speed during continuous rotation
                pivot_timeout = 0;
            ...
            //Cruise: speed-tiered P-controller wall following
            else {
                // Adjust base speed based on proximity to front wall
                dist_front = read_sonar_front();

                if(dist_front == 999 || dist_front > 50){
                    base_speed = 75;        // open corridor: full speed
                }
                else if(dist_front > 30){
                    base_speed = 55;        // approaching: moderate speed
                }
                else if(dist_front >= 22){
                    base_speed = 40;        // near threshold: slow down
                }
                else {
                    continue;               // below 22 cm: trigger avoidance next iteration
                }

                // Read side sonars for lateral error
                dist_mid  = read_sonar_mid();
                dist_left = read_sonar_left();
                ...
                // Deadband suppresses noise; correction reduces speed slightly
                if(p_error < (int)DEADBAND && p_error > -(int)DEADBAND){
                    p_error = 0;
                }
                else {
                    if(base_speed >= 30) {
                        base_speed = base_speed - 10;   // reduce speed during correction
                    }
                }

                // P-controller with symmetric correction cap
